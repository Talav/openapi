...
func TestView_ComprehensiveSpec(t *testing.T) {
	...
	assert.True(t, warnings.Has(debug.WarnDegradationWebhooks), "Should warn about webhooks")
	assert.True(t, warnings.Has(debug.WarnDegradationInfoSummary), "Should warn about info.summary")
	assert.True(t, warnings.Has(debug.WarnDegradationLicenseIdentifier), "Should warn about license.identifier")
	assert.True(t, warnings.Has(debug.WarnDegradationPathItems), "Should warn about pathItems")
	assert.True(t, warnings.Has(debug.WarnDegradationMultipleExamples), "Should warn about multiple examples")
	assert.True(t, warnings.Has(debug.WarnDegradationConstToEnum), "Should warn about const conversion")
	assert.True(t, warnings.Has(debug.WarnDegradationContentEncoding), "Should warn about contentEncoding")
	assert.True(t, warnings.Has(debug.WarnDegradationContentMediaType), "Should warn about contentMediaType")
	assert.True(t, warnings.Has(debug.WarnDegradationUnevaluatedProperties), "Should warn about unevaluatedProperties")

	// Create JSON from result and compare with expected
	jsonBytes, err := json.MarshalIndent(result, "", "  ")
	require.NoError(t, err)

	// Expected JSON for comprehensive spec
	expectedJSON := `{
  "openapi": "3.0.4",
  "info": {
    "title": "Test API",
...
func TestTransformSchema_Warnings(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode: debug.WarnDegradationMultipleExamples,
		},
		{
			name: "const value",
			schema: &model.Schema{
			...
			wantCode: debug.WarnDegradationConstToEnum,
		},
		{
			...
			wantCode: debug.WarnDegradationContentEncoding,
		},
		{
			...
			wantCode: debug.WarnDegradationContentMediaType,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, result)
			assert.True(t, warnings.Has(tt.wantCode), "Expected warning %s not found", tt.wantCode)
		})
	}
}
...
func createComprehensiveSpec() *model.Spec {
	return &model.Spec{
		Info: model.Info{
			...
			Description: "A test API",
			Version:     "1.0.0",
			Summary:     "This is a summary (3.1-only feature)", // triggers warning
			License: &model.License{
				Name:       "MIT",
				Identifier: "MIT", // 3.1-only feature, triggers warning
			},
		...
		Components: &model.Components{
			...
			PathItems: map[string]*model.PathItem{ // 3.1-only feature, triggers warning
				"common": {
					Parameters: []model.Parameter{
		...
		Webhooks: map[string]*model.PathItem{ // 3.1-only feature, triggers warning
			"userCreated": {
				Post: &model.Operation{
...
func createComplexSchema() *model.Schema {
	return &model.Schema{
		...
		// 3.1-only features to trigger warnings
		Examples: []any{
			map[string]any{"id": "1", "name": "Example 1"},
			map[string]any{"id": "2", "name": "Example 2"}, // triggers multiple examples warning
		},
		ContentEncoding:  "gzip",                        // triggers content encoding warning
		ContentMediaType: "application/json",            // triggers content media type warning
		Unevaluated:      &model.Schema{Type: "string"}, // triggers unevaluated properties warning
	}
}
...
func createConstSchema() *model.Schema {
	return &model.Schema{
		Title: "Status Constant",
		Const: "active", // triggers const warning
	}
}
